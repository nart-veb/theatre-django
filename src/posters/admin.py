...
from _project.settings import MEDIA_ROOT
from posters.models import Producer, Performance, PerformanceImage, Poster, Ticket, Map
...
class PosterAdmin(admin.ModelAdmin):
  def save_model(self, request, obj, form, change):
    ...
    if change == False:
      ...
      path = os.path.join(
                          MEDIA_ROOT, 
                          str(get_object_or_404(Poster.objects, id=request.POST["posters"]).perfomance.map.seats_json)
                          )
